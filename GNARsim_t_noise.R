...
GNARsim_t_noise <- function(n=200, net=GNAR::fiveNet, alphaParams=list(c(rep(0.2,5))), betaParams=list(c(0.5)), sigma=1,
  ...
  xx.init <- matrix(rt(nnodes*ntimedep, 9), nrow=ntimedep, ncol=nnodes)
...
net <- make_tree(n = 20, children = 3, mode = 'undirected')
net_gnar <- igraphtoGNAR(net)
ts_data <- GNARsim_t_noise(n = 200, net = net_gnar, alphaParams = alphaParams, betaParams = betaParams)
